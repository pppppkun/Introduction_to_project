...
## 请大家从头到尾看完，这对你完成这个项目非常有帮助，如何开始这个项目已经写在结尾了。
...
- 在项目目录中新建一个文件夹，命名为你github的用户名`<username>`
    - 在你的文件夹中新建一个`README.md`，并在里面写上你的姓名与学号
    - 然后将`src`中的文件全部拷贝到你的文件夹中。
    - 在你的文件夹中新建一个`SimpleAI.cpp`，然后在里面实现`SimpleAI.h`中定义的函数
- 完成`SimpleAI.h`中定义的函数和`main.cpp`中定义的函数
- 每当你完成一个小功能(在你自己的计划中)，我们非常建议你进行测试后push一下，做一下版本控制
    - 上传需要的指令有
    - `git add .`
    - `git commit -m '<commit_message>'`
    - `git push`
- 当你完成这个项目后，可以对我发起`Pull request`，也可以不，如果发起的话，我会把大家的代码合并到一个仓库中。
    - 注：11月10号前不接受任何`Pull  request`，为了让别人不把你写的代码也一起fork过去
- 如果你有任何问题，请在我的仓库中发起一个issue，我每天会去看，在同学们期中考完后（11.16）我会把我的代码push到我的仓库中，这样大家就可以参考我是怎么实现的。
- 当你完成这个项目后，希望你更新一下你的`README.md`，写一些个人总结，或是你认为你克服了哪些难题，你是如何克服的？
- 任何更新我都会在群里告诉大家
- DDL在十二月一号
